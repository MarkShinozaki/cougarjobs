    ...
    <main className="min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-6 py-16">
        ...
        >
          ...
          <p className="text-xl text-cougar-cream/60 max-w-2xl mx-auto mb-8 font-body">
          ...
          {features.map((feature) => (
            <motion.div key={feature.title} variants={item}>
              <Link href={feature.href} className="block h-full">
                <div className="h-full p-6 rounded-2xl glass card-hover group cursor-pointer">
                  <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${feature.color} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}>
                    <feature.icon className="w-7 h-7 text-white" />
                  </div>
                  <h3 className="font-display text-xl font-semibold mb-2">{feature.title}</h3>
                  <p className="text-cougar-cream/50 text-sm">{feature.description}</p>
                </div>
              </Link>
            </motion.div>
          ))}
        ...
        <motion.div
          ...
          className="mt-24 grid grid-cols-3 gap-8 max-w-3xl mx-auto text-center"
        >
          {[
            ...
            <div key={stat.label}>
              ...
              <div className="text-cougar-cream/40 text-sm mt-1">{stat.label}</div>
            </div>
          ))}
        </motion.div>
      </div>
